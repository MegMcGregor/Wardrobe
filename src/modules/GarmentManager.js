...
export const getAllGarments = () => {
  return fetch(`${remoteURL}/garments?_expand=color&_expand=type`)
    .then(res => res.json())
}

export const getGarmentsByUser = (userId) => {
  return fetch(`${remoteURL}/garments?userId=${userId}&_expand=type&_expand=color`)
    .then(res => res.json())
